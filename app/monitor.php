...
if (isset($_GET['api']) && $_GET['p'] === $password) {
    ...
    if (strpos(PHP_OS, 'WIN') === false) {
        ...
        if ($free) {
            ...
            // stats[1] = Total, stats[2] = Used
            $ram_text = $stats[2] . " MB / " . $stats[1] . " MB";
            // Kita ambil angka murni (Used) untuk chart
            $ram_chart_value = (int)$stats[2]; 
        }
    }
    ...
    echo json_encode([
        'waktu' => date('H:i:s'),
        'load' => $load[0],
        'ram_string' => $ram_text,         // Untuk tulisan
        'ram_numeric' => $ram_chart_value, // Untuk grafik
    ...
    <?php if($current_page == 'status'): ?>
        <div class="grid">
            <div class="card">
                ...
                <div id="load-val-cpu" class="value">0.00</div>
                <div class="chart-container">
                    <canvas id="loadChart"></canvas>
                </div>
            </div>
            <div class="card">
                <div class="label">Real-time RAM Load</div>
                <div id="load-val-ram" class="value">0.00</div>
                <div class="chart-container">
...
if (document.getElementById('loadChart')) {
    ...
    const loadChart = new Chart(ctxLoad, chartConfig('#58a6ff'));
    const ramChart = new Chart(ctxRam, chartConfig('#39ff14')); // Warna hijau untuk RAM

    function updateSystemData() {
    fetch(`?p=<?php echo $password; ?>&api=1`)
        .then(res => res.json())
        .then(data => {
            // 1. Update Teks di Dashboard
            document.getElementById('load-val-cpu').innerText = data.load.toFixed(2);
            document.getElementById('load-val-ram').innerText = data.ram_numeric + " MB"; // Tampilkan angka di card
            document.getElementById('ram-val').innerText = data.ram_string; // Tampilkan Full Info di tabel
            ...
            // 3. Update RAM Chart (Menggunakan data numeric)
            ramChart.data.labels.push(data.waktu);
            ramChart.data.datasets[0].data.push(data.ram_numeric);

            // Batasi data agar tidak memenuhi memori browser (max 25 poin)
            if(loadChart.data.labels.length > 25) {
                [loadChart, ramChart].forEach(c => {
                    c.data.labels.shift();
